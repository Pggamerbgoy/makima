...
Now supports recursive prompt loading from 'promts for coding/' and
multi-turn tool execution via EditorBridge.
"""
import json
import os
import re
...
CODING_PROMPTS_DIR = os.path.join(root_dir, "promts for coding")

# Maximum number of tool-call turns before stopping (safety limit)
MAX_TOOL_TURNS = 10
...
class ChatPanel(QWidget):
    ...
    tool_request = pyqtSignal(str, dict)   # tool_name, arguments
    ...
    def __init__(self, parent=None):
        ...
        self._worker = None
        self._bridge = None  # Set by MainWindow via set_bridge()
        self._tool_turn_count = 0
        self._setup_ui()

    def set_bridge(self, bridge):
        """Attach the EditorBridge for tool execution."""
        self._bridge = bridge
    ...
    def _start_api_call(self):
        """Create a new ApiWorker and start it on a QThread."""
    ...
    def _cleanup_worker(self):
        """Safely stop and discard the previous worker thread."""
    ...
    def _on_response(self, text):
        ...
        # Check if the response contains a tool/function call
        tool_call = self._parse_tool_call(text)
        if tool_call and self._bridge and self._tool_turn_count < MAX_TOOL_TURNS:
            tool_name, tool_args = tool_call
            self._tool_turn_count += 1

            # Show tool call in chat
            self._add_tool_bubble(tool_name, tool_args)

            # Execute via bridge
            result = self._bridge.dispatch(tool_name, tool_args)

            # Show result
            self._add_tool_result_bubble(tool_name, result)

            # Send result back to AI for next turn
            self.messages.append({"role": "assistant", "content": text})
            self.messages.append({"role": "user", "content": f"[Tool Result for {tool_name}]:\n{result}"})
            self._start_api_call()
            return

        # Normal text response — show it
        self._tool_turn_count = 0
        self.messages.append({"role": "assistant", "content": text})
    ...
    def _parse_tool_call(self, text: str):
        """
        Extract a tool/function call from AI response text.
        Supports multiple formats used by different AI models.
        """
        # Format 1: JSON function_call block
        ...
        # Format 2: Embedded JSON in markdown code block
        # ```json\n{"name": "read_file", ...}\n```
        ...
        # Format 3: Simple tool call pattern
        # <tool_call>read_file({"target_file": "main.py"})</tool_call>
    ...
    def _continue_conversation(self):
        """Send the accumulated messages back to the AI for the next turn."""
        self._start_api_call()

    def _add_tool_bubble(self, tool_name: str, args: dict):
        """Show a collapsible tool call indicator in the chat."""
        t = get_theme()
        args_short = json.dumps(args, indent=2)[:200]
        content = f"🔧 Calling: {tool_name}\n{args_short}"
        bubble = MessageBubble("assistant", content)
        bubble.setStyleSheet(f"""
            QFrame {{
                background: {t.get('bg3', t['bg'])};
                border-radius: 8px;
                border: 1px dashed {t.get('accent', '#007acc')};
                margin: 2px 30px 2px 2px;
            }}
        """)
        self.chat_layout.insertWidget(self.chat_layout.count() - 1, bubble)

    def _add_tool_result_bubble(self, tool_name: str, result: str):
        """Show the tool execution result in chat."""
        t = get_theme()
        ...
        bubble.setStyleSheet(f"""
            QFrame {{
                background: {t.get('bg3', t['bg'])};
                border-radius: 8px;
                border: 1px solid {t.get('green', '#4ec9b0')};
                margin: 2px 30px 2px 2px;
            }}
        """)
        self.chat_layout.insertWidget(self.chat_layout.count() - 1, bubble)
    ...
    def _load_prompts_dropdown(self):
        """Scan both prompt directories recursively and populate dropdown."""
        # 1. Flat scan of MAKIMA LAUNCHER/prompts/
        self._scan_prompts_dir(PROMPTS_DIR, prefix="")

        # 2. Recursive scan of promts for coding/ (subfolders = categories)
        self._scan_prompts_dir(CODING_PROMPTS_DIR, prefix="", recursive=True)

    def _scan_prompts_dir(self, base_dir: str, prefix: str = "", recursive: bool = False):
        """Scan a directory for .txt prompt files. Subfolders become categories."""
        ...
        for entry in entries:
            ...
            if os.path.isdir(full_path) and recursive:
                # Subfolder = category (e.g. "cursor", "claude")
                category = entry.replace("_", " ").title()
                new_prefix = f"{prefix}{category} / " if prefix else f"{category} / "
                self._scan_prompts_dir(full_path, prefix=new_prefix, recursive=True)
                ...
                try:
                    with open(full_path, "r", encoding="utf-8") as f:
                        ...
                        if content:
                            self.prompt_combo.addItem(display_name, content)
                except Exception as e:
                    print(f"Failed to load prompt {entry}: {e}")

            elif entry.endswith(".json"):
                # Load JSON tool definitions (store as data, not as system prompt)
                name = os.path.splitext(entry)[0].replace("-", " ").replace("_", " ").title()
                display_name = f"{prefix}{name} [Tools]" if prefix else f"{name} [Tools]"
                try:
                    with open(full_path, "r", encoding="utf-8") as f:
                        content = f.read().strip()
                        if content:
                    ...
                    print(f"Failed to load tool def {entry}: {e}")
